...
#TAU fraction matrix (MAX)
my $tau_file = "/NGS/users/Sid/ChromHmm/Posterior/State_6/POSTERIOR/Tau_expression/Ens+refGene_mm9/results/Tau_fractions/tau_fraction_max.txt" ;
...
my %tau ;
open(ta,$tau_file) or die "Cannot open $tau_file" ;
foreach(<ta>){
	$_ =~ s/\n|\r|\s+$//g ;
	my($id,$gene,$rest) = split("\t",$_,3) ;
	$tau{$gene} = $rest ;
}
close ta ;
